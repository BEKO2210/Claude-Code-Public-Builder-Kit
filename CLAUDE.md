...
2. **Run #025 — Logo + landing-page refit.** The hosted app's success path is now usable end-to-end by a non-technical user (#022 / #022b-c / #023 / #024). The landing page (`docs/`) still reads as a wall of text and uses the same compass-bloom logo. Run #025 ships a distinct mark, a hero with the live demo embedded (not just a link to it), a three-image "how it works" strip, and one example output above the fold. Decide the logo direction with the owner before any pixel is committed.
