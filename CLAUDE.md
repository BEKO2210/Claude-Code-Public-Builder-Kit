...
1. **Run #026 — Deutsche Sprachvariante** (UI copy across wizard, post-generate panel, landing page). Owner is German-speaking and so is part of the target audience; everything user-facing is currently English. Decide whether to default-detect from `Accept-Language` or default-English with a header switcher.
2. **Domain depth: resume at eleventh domain** (`non-profit & community` or `government & civic`). The remaining ~7 specialisations stay valuable.
