...
1. **Domain depth: sixth domain.** Five domains now specialised (`climate & sustainability`, `professional services`, `health & wellness`, `finance`, `food & hospitality`). Best next candidates: **`education`** (FERPA / age-of-consent / proctoring + parent-vs-student data, accessibility for diverse learners, tutor-not-replacement framing), or **`logistics & supply chain`** (driver UX, hardware integration, telematics privacy, peak-season reliability).
