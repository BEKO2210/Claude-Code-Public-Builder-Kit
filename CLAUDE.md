...
1. **Domain depth: seventh domain (`logistics & supply chain`).** Six domains now specialised (`climate & sustainability`, `professional services`, `health & wellness`, `finance`, `food & hospitality`, `education`). Logistics is next: driver UX (single-handed phone use behind the wheel where legal, voice-first where required), hardware integration (ELD / GPS units / temperature sensors / barcode scanners — failure modes are not just software bugs), telematics privacy (driver location is personal data under GDPR; in the US, the ELD mandate produces an audit trail that's both regulatory ammunition *and* a privacy surface), peak-season reliability (Q4 retail + holiday capacity collapses are the operational test), HOS / DOT / ELD regulatory surface (FMCSA in the US, EU 561/2006 + driving-time rules in Europe).
