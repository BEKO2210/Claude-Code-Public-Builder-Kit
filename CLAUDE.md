...
2. **Run #023 — Wizard-Onboarding.** Replace the single textarea + Generate button with 4–5 friendly steps in plain language: "Was möchtest du erschaffen?" (icon picker), "Für wen?" (suggestions), "Was soll es besser machen?" (free text), "In welcher Region?" (dropdown), live preview of the inferred context next to each step. Substantially lowers the floor for users who don't already know what they want to type. Lives in `public/`; hosted version benefits automatically.
3. **Run #024 — "Was mache ich jetzt damit?" post-generate guidance.** After generation, instead of dumping 12 files in front of a non-technical user, add a 3-step "use your kit" panel: (a) Download ZIP, (b) Go to claude.ai and start a chat, (c) Drop in `MASTERPLAN.md` and paste a starter prompt. Includes screenshots. Removes the "Claude Code / Terminal" assumption from the success path.
