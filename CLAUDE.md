...
├── tests/
│   └── generator.test.js     # node:test suite (105 tests, no external deps)
...
- [ ] `npm test` passes (currently 105 tests).
...
**Phase shift after Run #022.** The reach work is now complete (#022 hosted, #022b/c hardened, #023 wizard, #024 post-generate panel, #025 logo + landing-page refit). A non-technical user can land on the GitHub Pages page, click **Launch the tool**, complete the wizard, get a kit, and follow a 3-step path into Claude — all without a terminal. The shortlist below reflects what's left.

1. **Domain depth: resume at the eleventh domain** (`non-profit & community` or `government & civic`). Reach pivot is done after Run #026 — output substance is the natural next priority. The remaining ~7 specialisations were paused after Run #021; same shortlist resumes.
2. **Per-locale URLs / SEO** for the German landing — `/de/index.html` mirror so Google indexes both. Only worthwhile if German organic traffic becomes a stated goal.
3. **Optional file-tree filter** for `#file-list` — only worthwhile once examples grow beyond 12 files.

Whichever you pick, file an entry in `RUN_LOG.md` first.

### Recently completed

- ✓ **Run #039 — Wizard erfasst jetzt die Domain-tragende Sache („Worum geht es?") + deutsche Keyword-Erkennung.** Fix für den in Run #038 diagnostizierten Mangel: der Wizard fragte **Was / Wer / Warum**, aber nie *worum es geht*, sodass z. B. ein Bio-Laden auf `domain: general` fiel und die `retail & e-commerce`-Spezialisierung nie auslöste. Jetzt 5 Schritte (**Was → Worum → Wer → Warum → Erzeugen**) mit neuem Pflicht-Feld „Worum geht es?" (6 Pills, Nudge). Komposition umgebaut auf `"<opener> for <subject>. Built for <audience>, it <benefit>."` — das `Built for` trifft bewusst das höher priorisierte Audience-Pattern, sodass die `for …`-Klausel des Subjects nicht als Audience missverstanden wird. `deriveProjectName` nimmt nur den ersten Satz (saubere Namen bei Mehrsatz-Ideen; Beispiele unberührt, da einsätzig). Mobile-Wunsch über neue Benefit-Pill abgedeckt. Deutsche Keywords für retail / food / health / education / professional services / real estate / finance (z. B. `bioladen`, `hofladen`, `zahnarztpraxis`, `fahrschule`, `anwaltskanzlei`, `immobilien`) ergänzt — frei eingetippte deutsche Ideen landen jetzt im richtigen Bereich. EN + DE vollständig übersetzt. „other"-Pfad fixt doppelten Artikel („An a Slack bot"). 105/105 Tests (+9), 0 a11y-Violations, zero Beispiel-Drift, sync:assets in sync. See Run #039.
- ✓ **Run #026 — Deutsche Sprachvariante (i18n auf beiden Pages).** Auto-detect via `navigator.language` + manual `EN | DE` switcher in the header (pill-style segment control). Two parallel i18n implementations, both dependency-free / build-step-free / no-server-change: `public/i18n.js` (ESM module, ~165 keys per locale, walks `[data-i18n*]` attributes, exposes `t/setLocale/applyTranslations/onLocaleChange`); `docs/i18n.js` (IIFE script with `defer`, ~50 keys per locale, same convention). Both persist user choice in `localStorage`. Translated: wizard end-to-end (steps, pills, nav buttons, summary, persist label), direct form, result panel + `Use your kit in 3 steps` + Copy-prompt + the **starter prompt** itself (the string Claude consumes), gallery cards, status messages, source badge, `Detected: …`. Landing page: nav, hero, how-it-works, example-section, what-you-get, devs, footer. App.js subscribes to locale change and re-renders gallery + project-aware prompt. 81/81 tests pass; 0 a11y violations on either page. The composed-sentence pattern stays English (`for X. It Y.`) because the kit's *output* is English — only the UI translates. Vater-Test now passes end-to-end in German. See Run #026.
- ✓ **Run #025 — Logo redesign (Open Plan / Faltblatt) + landing-page refit + live URL linked from `docs/`.** New mark: three sheets fanning out behind a structured front page with a heading + content lines + section dividers. Story: one idea unfolding into a complete planning document set. Three layered animations (aura breathe 5.6 s, title pulse 4.8 s, fan oscillation 6.4 s), all `prefers-reduced-motion`-respecting. `public/logo.svg`, `public/logo-monochrome.svg`, `public/favicon.svg` rewritten and mirrored to `docs/`. `docs/index.html` rebuilt around: two-column hero with eyebrow tag + 60-px headline + plain-language lede + primary CTA "**Launch the tool →**" linking to `https://claude-code-public-builder-kit.vercel.app/` + trust row; faux-app-window hero card showing the input → output story; three-step "How it works" strip with mock visuals (tile picker, document stack, claude.ai chat); real MASTERPLAN.md snippet above the fold with a soft fade; condensed 12-file grid below; "For developers" section with curl pointing at the Vercel URL. Old "Why this exists" prose removed — the lede + trust row carry that work now. OG card rebuilt (`docs/og-source.svg` → `docs/og-card.png`, 179 KB) with the new logo + new headline. **Live URL is linked from four places in `docs/index.html`** — closes the Run #022 follow-up. 81/81 tests pass; 0 a11y violations on either page (37/27 axe rules); all 13 WCAG-AA contrast pairs pass; sync:assets:check is in sync. See Run #025.
- ✓ **Run #024 — "Was mache ich jetzt damit?" post-generate panel.** Between the result-header and a now-collapsed file viewer, three numbered cards walk the user through Download ZIP → Open Claude (claude.ai/new in a new tab) → Attach MASTERPLAN.md and paste a project-aware starter prompt. The prompt is built per-result (`buildStarterPrompt(title)`), one short paragraph asking Claude to summarise the masterplan back, then walk the user through Phase 1 / Step 1 in plain language, one question at a time. Copy-prompt button via Clipboard API. The file viewer drops to `<details>` / `<summary>` (default-closed) — power users can still peek, but the success flow no longer leads with "here are 12 files, figure it out". Pure UI change in `public/`; no server, template, or test changes. 81/81 tests pass; 0 a11y violations; all 13 contrast pairs pass WCAG AA. See Run #024.
